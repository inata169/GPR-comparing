...
- **Tier 2 (ユーザー体験)**: [ ] Web GUI化 (クロスプラットフォーム), [ ] マルチプレーン/DVH 同時表示, [x] トレンド解析DB保存, [x] JSON設定プリセット管理, [x] **GUI に PDF 出力ボタンを追加** 
- **Tier 3 (高度解析)**: [x] ガンマヒストグラム・空間分析, [ ] 多基準同時評価並行実行, [x] サードパーティ(3DVH)との相互検証分析, [ ] 不確かさのブートストラップ推定, [ ] MHD/NRRD対応
- **Tier 4 (運用・配布)**: [x] `.exe` パッケージビルド実装 (`build_exe.ps1`), [x] 最小構成配布パッケージZIP自動化 (`package_release.ps1`), [ ] 完全日英多言語化 (i18n), [ ] プラグイン機構, [ ] 監査・コンプライアンス対応
...
- [x] **3D ガンマビューアの実装**
  ...
  - [x] **GUIへの3Dビューア完全統合** (`run_gui.ps1`)
    - [x] Action 選択への追加と、matplotlibウィンドウを殺さずにリダイレクトなしで起動する `Run-Viewer` 実装。
    - [x] ビューア表示の物理アスペクト比(1:1)および Axial 上下反転（医療画像慣習）への対応。
    - [x] GUI 側での CT Directory 指定と設定保存。
