...
- **Tier 2 (ユーザー体験)**: [ ] Web GUI化 (クロスプラットフォーム), [ ] マルチプレーン/DVH 同時表示, [x] トレンド解析DB保存, [x] JSON設定プリセット管理, [x] **GUI に PDF 出力ボタンを追加** (`run_gui.ps1` に `--pdf` オプションとチェックボックスを追加し、ワンクリックで PDF レポートを生成できるようにする)
