...
PyMedPhys 0.41.0 is the reference gamma engine and the omitted-value default in the CLI and batch runner. The GUI defaults to the parallel Numba engine for practical full-volume GPR runtimes; PyMedPhys remains explicitly selectable for controlled reference calculations. See the [PyMedPhys standardization change](docs/openspec/changes/pymedphys-standard-engine/proposal.md).
...
Different non-empty `FrameOfReferenceUID` values fail closed by default. If header comparison confirms that anonymization alone changed the UID and both RTDOSE files still use the same absolute DICOM patient coordinates, the operator may explicitly add `--allow-frame-of-reference-mismatch`. The GUI exposes the same choice as `Allow different FoR UID` and asks for confirmation before each analysis or Viewer launch. The override does not align, register, or shift the inputs; it only permits the existing world-coordinate resampling. Reports preserve both UIDs, `same_for_uid=false`, the enabled override in provenance, and a prominent warning. Do not use the override for unrelated studies or coordinate frames.

## 7. GUI usage

On Windows, start source mode with `run_gui_python.bat` or run `scripts/run_gui.ps1`. The GUI accepts reference and evaluation RTDOSE files, an output directory, optional RTSTRUCT/ROI values, DD, DTA, cutoff, normalization, a preset, gamma engine, 2D plane/index, thread count, shift optimization, local gamma, sub-voxel interpolation, an explicit verified-anonymization Frame of Reference override, PDF/NPZ/SQLite/log options, and the requested action.

The actions are Header Compare, 3D Gamma, 2D Gamma, and 3D Viewer. The GUI builds arguments for the same `rtgamma.main` CLI and always passes the selected engine. `Numba (fast full-volume GPR)` is selected initially with interpolation fraction 4; `PyMedPhys (reference / slow 3D)` remains available and displays a warning before full 3D execution. The value is persisted locally as `Gamma/engine` in the Git-ignored `config/gui_config.ini`. When that file is absent, the GUI reads the tracked `config/gui_config.example.ini`, whose workstation paths are blank. Do not distribute locally saved DICOM paths. An older GUI configuration without the engine key migrates to Numba and shows a warning. Every GUI 3D Gamma run writes `gamma3d.npz`, `diff3d.npz`, and `run3d.json` to the selected output folder for validated Viewer reuse. Gamma and Pass/Fail require that cache; Ref/Eval Dose, Dose Diff, and Dose Ratio remain available from the DICOM inputs.
...
Supply `--rtstruct` and repeat `--roi` to select structures; omit `--roi` to process all contours found. Masks are generated on the reference RTDOSE grid. Reports include voxel count, evaluated count, ROI GPR, gamma statistics, and reference/evaluation DVH statistics. Paired reference/evaluation DVHs use the same finite voxel support. If an ROI contains no paired finite dose values, reporting completes with an empty DVH and unavailable statistics instead of discarding the completed Gamma result.
...
The project owner approved the documented validation scope, numerical policy, known limitations, and v0.9.3 source publication on 2026-08-11. Source-only v0.9.4 superseded it on 2026-08-12. Version 0.9.5 incorporates the subsequently accepted Frame of Reference override workflow and common-spatial evaluation rule. Version 0.9.6 adds robust paired-DVH handling for partially or entirely unavailable evaluation-dose ROI support; see the [v0.9.6 release notes](docs/RELEASE_NOTES_0.9.6.md). These records are software-verification evidence, not clinical-use approval or patient-dose acceptance.

The RTDOSE loader validates IPP, IOP, Pixel Spacing, GFOV, dimensions, Dose Grid Scaling, Dose Units, and finite dose values before calculation. It accepts strictly ascending or descending GFOV and sorts frames and offsets together into ascending order. Axial absolute-z GFOV is converted to offsets from IPP. Different origins and voxel spacing are supported. Differing orientations and Dose Units fail closed. Present but unequal Frame of Reference UIDs also fail closed by default; the CLI, GUI, batch runner, and Viewers expose an explicit override only for cases where the operator has independently verified a shared patient coordinate system and a UID-only difference such as anonymization. The override does not perform spatial registration. The Numba engine also fails closed when an evaluation axis is nonuniform because its kernels require constant evaluation-grid spacing; select PyMedPhys or resample the evaluation RTDOSE instead.
...
The Windows offline bundle has automated integrity and licensing checks plus explicit PyMedPhys 0.41.0 and Numba smoke paths. Historical v0.9.4 and PR #29 candidate ZIPs do not contain the final v0.9.5 source and must not be redistributed under another version. Build a new distribution only from its approved tag and record ZIP integrity, SHA-256, embedded identity, licensing coverage, and synthetic packaged-runtime engine checks. For v0.9.6, the project owner approved treating the subsequent TPS-PC step as a release-candidate EXE operational check rather than repeating the completed fixed-condition source-workflow study as a separate formal packaged acceptance. This scope decision does not establish clinical suitability or waive package verification before transfer.
