...
The project owner approved the documented validation scope, numerical policy, known limitations, and v0.9.3 source publication on 2026-08-11. Source-only v0.9.4 superseded it on 2026-08-12 after the offline TPS/Viewer fixes and follow-up review corrections were merged. This is not clinical-use approval and does not establish a clean-machine-verified offline installation. See the [release-readiness checkpoint](docs/RELEASE_READINESS_2026-08-11.md) and the [v0.9.4 progress and handoff record](docs/PROGRESS_2026-08-12.md).

The RTDOSE loader validates IPP, IOP, Pixel Spacing, GFOV, dimensions, Dose Grid Scaling, Dose Units, and finite dose values before calculation. It accepts strictly ascending or descending GFOV and sorts frames and offsets together into ascending order. Axial absolute-z GFOV is converted to offsets from IPP. Different origins and voxel spacing are supported. Differing orientations, Dose Units, or present Frame of Reference UIDs fail closed; a Frame of Reference mismatch requires an explicit spatial-registration workflow that is not currently implemented. The Numba engine also fails closed when an evaluation axis is nonuniform because its kernels require constant evaluation-grid spacing; select PyMedPhys or resample the evaluation RTDOSE instead.
...
The Windows offline bundle has automated integrity and licensing checks plus explicit PyMedPhys 0.41.0 and Numba smoke paths. A Python 3.12 x64 wheelhouse/ZIP built before v0.9.4 passed a local `--no-index` bundle smoke test; its digest and limitations are recorded in the [2026-08-11 progress record](docs/PROGRESS_2026-08-11.md). That ZIP does not represent the final v0.9.4 source. Rebuild it from the v0.9.4 tag and repeat the TPS-PC Gamma, report/PDF, and Viewer acceptance checks before binary distribution. A physically network-isolated installation and clean Windows/Python-absent acceptance run also remain pending; these are not blockers for the source-only Release.
