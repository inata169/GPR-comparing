...
The actions are Header Compare, 3D Gamma, 2D Gamma, and 3D Viewer. The GUI builds arguments for the same `rtgamma.main` CLI and always passes the selected engine. `PyMedPhys (standard)` is selected initially; `Numba (legacy / experimental)` is available for explicit reproduction work. The value is persisted as `Gamma/engine` in `config/gui_config.ini`. The tracked seed configuration keeps all workstation file paths blank; do not commit DICOM paths written by Save Settings or external automation. An older GUI configuration without the engine key migrates to PyMedPhys and shows a warning. The 3D Viewer launch path uses the Fast Viewer. It can load CT, RTDOSE, RTSTRUCT, and a precomputed `gamma3d.npz` containing the `gamma` array.
